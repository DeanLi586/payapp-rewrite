<template>
  <div>
    <h1 class="align-center mb-5">Dashboard</h1>

    <v-layout class="info ma-2">
      <v-flex row wrap class="xs-12 md-4">
        <v-card transition="fade-transition" hover class="ma-2 cyan accent-3">
          <v-card-text class="align-center justify-center">
            <v-icon>shopping_cart</v-icon>
          </v-card-text>
          <v-card-title class="display-1 justify-center">Products</v-card-title>
          <v-card-text class="text-xs-center justify-center">List of Products</v-card-text>
        </v-card>
      </v-flex>

      <v-flex row wrap class="xs-12 md-4">
        <v-card transition="fade-transition" hover class="ma-2 green accent-3">
          <v-card-text class="align-center justify-center">
            <v-icon>shopping_cart</v-icon>
          </v-card-text>
          <v-card-title class="display-1 justify-center">Products</v-card-title>
          <v-card-text class="text-xs-center justify-center">List of Products</v-card-text>
        </v-card>
      </v-flex>

      <v-flex row wrap class="xs-12 md-4">
        <v-card hover transition="fade-transition" class="ma-2 amber accent-3">
          <v-card-text class="align-center justify-center">
            <v-icon>shopping_cart</v-icon>
          </v-card-text>
          <v-card-title class="display-1 justify-center">Products</v-card-title>
          <v-card-text class="text-xs-center justify-center">List of Products</v-card-text>
        </v-card>
      </v-flex>
    </v-layout>

    <v-layout class="success ma-2">
      <v-flex row wrap class="xs-12 md-6 xl-12">
        <v-card hover class="ma-2">
          <v-card-text class="align-center justify-center">
            <v-icon>shopping_cart</v-icon>
          </v-card-text>
          <v-card-title class="display-1 justify-center">Products</v-card-title>
          <v-card-text class="text-xs-center justify-center">List of Products</v-card-text>
        </v-card>
      </v-flex>

      <v-flex row wrap class="xs-12 md-6 xl-12">
        <v-card hover class="ma-2">
          <v-card-text class="align-center justify-center">
            <v-icon>shopping_cart</v-icon>
          </v-card-text>
          <v-card-title class="display-1 justify-center">Products</v-card-title>
          <v-card-text class="text-xs-center justify-center">List of Products</v-card-text>
        </v-card>
      </v-flex>
    </v-layout>

    <v-layout class="warning ma-2">
      <v-flex row wrap class="xs-12 md-6 xl-12">
        <v-card hover class="ma-2">
          <v-card-text class="align-center justify-center">
            <v-icon>shopping_cart</v-icon>
          </v-card-text>
          <v-card-title class="display-1 justify-center">Products</v-card-title>
          <v-card-text class="text-xs-center justify-center">List of Products</v-card-text>
        </v-card>
      </v-flex>
    </v-layout>

    <v-layout class="warning ma-2">
      <v-flex row wrap class="xs-12 md-6 xl-12">
        <v-card hover class="ma-2">
          <v-card-text class="align-center justify-center">
            <v-icon>shopping_cart</v-icon>
          </v-card-text>
          <v-card-title class="display-1 justify-center">Products</v-card-title>
          <v-card-text class="text-xs-center justify-center">List of Products</v-card-text>
        </v-card>
      </v-flex>
    </v-layout>
  </div>
</template>

<script>
export default {};
</script>

<style>
</style>
